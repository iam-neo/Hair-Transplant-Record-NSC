...
function changePassword_(u,p){if(!p.currentPassword||!p.newPassword)throw clientError_('Current password and new password are required.');if(p.newPassword.length<8)throw clientError_('New password must be at least 8 characters.');if(hashPassword_(p.currentPassword,u.PasswordSalt)!==u.PasswordHash)throw clientError_('Current password is incorrect.','UNAUTHORIZED');var newSalt=Utilities.getUuid();updateRow_('Users','UserID',u.UserID,{PasswordHash:hashPassword_(p.newPassword,newSalt),PasswordSalt:newSalt});audit_(u,'','UPDATE','User',u.UserID,'User changed their own password');return ok_({message:'Password updated successfully.'});}
function updateProfile_(u,p){if(!p.fullName||!p.username)throw clientError_('Full name and username are required.');var cleanUsername=String(p.username).trim();var existing=find_('Users','Username',cleanUsername);if(existing&&existing.UserID!==u.UserID)throw clientError_('Username is already taken.');updateRow_('Users','UserID',u.UserID,{FullName:String(p.fullName).trim(),Username:cleanUsername});audit_(u,'','UPDATE','User',u.UserID,'User updated their profile information');var updated=find_('Users','UserID',u.UserID);return ok_({user:publicUser_(updated)});}
